...
| # | Conflict | Options |
|---|---|---|
| **Q1** | **Movement rate vs 10-min rounds.** Phase 1: movementRange 3 tiles @ 25m = 75m per turn. At 10 min/turn that's 7.5 m/min — real infantry march ~80–100 m/min. As written, crossing the 1km map takes ~2 hours of game time and 13+ turns of dead play. | (a) In-contact turns keep short cautious ranges but out-of-contact movement fast-forwards multiple "ticks" per player turn (fits your elastic-time instinct); (b) raise movementRange to realistic scale (~30–40 tiles/10min unencumbered) and let engagement proximity slow it; (c) decouple: movement turns and combat rounds are different clocks. My lean: (a) — smallest change to working code, matches locked decision 1. |
| **Q2** | **Old code disposition.** §7 proposes moving old src to `legacy/` in-repo. Alternatives: orphan `archive` branch, or a separate `cohort-archive` repo. In-repo is safest against loss but sits where an overzealous agent could touch it. | legacy/ folder, archive branch, or separate repo? |
| **Q3** | **Elite death-chance table** (15% recruit → 6% legendary per qualifying battle). At those rates a 10-officer roster averages ~1–1.5 deaths *per battle* — a fully legendary roster is near-impossible to assemble (surviving 10+ battles at compounding ~10% each ≈ 35% per officer, and the roster churns constantly). That may be *intended* brutality (knowledge is fragile) or an old-doc number that fights the battle-3/5/10 naming arc. | Keep as-is and let the harness show the survival curves, or pre-commit to gentler rates? My lean: keep, simulate in Phase 6, tune with data. |
| **Q4** | **`effectiveness` semantics** (recovered plan §7.1). The weapon-vs-armor-class tables (0–100 per target class) are the rock-paper-scissors core, but nothing defines whether 85 means a damage multiplier (×0.85), a hit chance, or armor penetration. Everything downstream follows from this. | My lean: damage multiplier (÷100) — smoothest fit with the round-based casualty math; hit-chance adds a second RNG channel competing with chaos. |
| **Q5** | **Who attacks?** The harness deliberately reports engagements as an unordered a/b pair; attacker/defender is a design decision (recovered plan §7.2). Interacts with charge bonuses, prepared state, and terrain-holder advantage. | My lean: the unit that *entered* contact this turn is the attacker (charge/momentum applies); if contact persists from a prior round, both sides resolve symmetrically and only prepared/terrain asymmetries remain. |
| **Q6** | **Unit size vs tile capacity.** Army builder: 400 warriors/unit (elites contradictorily 300 or 40–100 in two live code paths). Converted tile capacity at 25m: ~100 standard. A 400-man unit doesn't fit on one tile. | (a) Units occupy multiple tiles (frontage becomes real — big design win, big movement complexity); (b) shrink standard units to ~100 (armies become 4–6 units of 100, matches your per-100 balancing instinct); (c) keep abstract 1-tile units and re-derive capacity/stacking as pure abstractions. My lean: (b) now, (a) as a Phase 8 ambition. Also pick ONE elite size. |
| **Q7** | **Tribal Warriors stats.** Locked decision 7 + notebook: same cost as Militia, same base stats, better morale. Legacy `TROOP_QUALITY` table: tribal 4 SP with 3/2/2 vs militia 5 SP with 2/2/1 — cheaper AND stronger on attack, which the recovered plan itself flags as the degenerate SP-efficient pick. | Notebook wins (equalize cost + stats, morale is the differentiator, faction-gated) — confirm, since it means editing a "tuned" legacy table. |
| **Q8** | **The separate `training` purchase** (none/basic/technical/expert, up to 6 SP) in the army builder has **no combat effect anywhere** — players pay for a dead field. The notebook model folds training into the quality tier. | Drop the separate purchase (quality tier IS training), or give it a real effect (e.g., stamina pool per locked decision 2)? My lean: fold into quality, refund the SP design space. |
