// src/phase1-movement/mapData.js
// Single source of truth for the battlefield.
// Named cells get a full entry. Everything else defaults to plains.

const CELLS = {
  "W6": {
    terrain: "ford",
    tags: ["ford", "river_crossing", "shallow_water"],
    landmark: "North Ford",
    aliases: ["north ford", "northern ford", "upper crossing", "the ford"],
    description: "A shallow gravel crossing where the river narrows between two mudflats"
  },
  "H15": {
    terrain: "bridge",
    tags: ["bridge", "river_crossing", "road", "structure"],
    landmark: "Harrow Bridge",
    aliases: ["the bridge", "harrow bridge", "harrow crossing", "stone bridge"],
    description: "An ancient stone bridge, wide enough for four men abreast"
  },
  "U21": {
    terrain: "ford",
    tags: ["ford", "river_crossing"],
    landmark: "Miller's Crossing",
    aliases: ["central ford", "millers crossing", "the central crossing", "middle ford"],
    description: "A wide but deep ford used by grain wagons — slow going for heavy troops"
  },
  "O34": {
    terrain: "ford",
    tags: ["ford", "river_crossing"],
    landmark: "South Ford",
    aliases: ["south ford", "southern ford", "lower crossing"],
    description: "A narrow ford at the river's bend, flanked by marsh on both sides"
  },
  "Q1": {
    terrain: "hill",
    tags: ["hill", "high_ground", "ridge", "elevation"],
    landmark: "The Crownhill",
    aliases: ["crownhill", "the crown", "northern heights", "north ridge", "the ridge"],
    description: "A broad hill commanding views across the entire northern approach"
  },
  "A20": {
    terrain: "hill",
    tags: ["hill", "high_ground", "ridge", "western_flank"],
    landmark: "Shepherd's Ridge",
    aliases: ["shepherds ridge", "western ridge", "the western heights", "the ridge"],
    description: "A long ridge running north-south along the western edge of the battlefield"
  },
  "F7": {
    terrain: "road",
    tags: ["road", "trade_route", "northern_road"],
    landmark: "The Merchant Road",
    aliases: ["merchant road", "the road", "northern road", "main road"],
    description: "A well-worn trade road running the length of the battlefield"
  }
}

// Phase 1 uses road: 0.5 so roads are meaningfully faster
...
// Terrain arrays copied verbatim from createTacticalMap() in mapUtils.js
const TERRAIN_MAP = {
  river: [
...
// Lookup terrain for a coordinate.
// CELLS entries are authoritative; fall back to TERRAIN_MAP arrays.
function getTerrainType(coord) {
  if (CELLS[coord]) return CELLS[coord].terrain
  for (const [terrainType, coords] of Object.entries(TERRAIN_MAP)) {
    if (coords.includes(coord)) return terrainType
  }
  return 'plains'
}

// Full cell data — terrain + tags + landmark + description
function getCell(coord) {
  if (CELLS[coord]) return CELLS[coord]
  return {
    terrain: getTerrainType(coord),
    tags: [],
    landmark: null,
    aliases: [],
    description: null
...
module.exports = { CELLS, MOVEMENT_COSTS, TERRAIN_MAP, getCell, getTerrainType }
